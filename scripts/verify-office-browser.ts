...
async function main() {
  ...
  try {
    ...
    // ============ LAST ON PURPOSE ======================================
    //
    // This started as section 1b, immediately after the seven views were
    // counted, which is where it reads best and is the wrong place for it.
    // Selecting Understanding starts the on-demand deep-knowledge load, and
    // every section below inherited both that load and a non-default view.
    // The suite went green with the section removed and red with it present,
    // failing eleven sections later on clicks that had nothing to do with the
    // rail — twice at the same reopen, and once on the rail's own geometry.
    //
    // A check that changes what the checks after it are looking at is not a
    // check, it is a mutation. It runs last on the phone instead, where the
    // only thing it can disturb is the desktop pass that reloads anyway.
    //
    ...
    // Existence was never the question. Reach was.
    {
      ...
      // ITS OWN STARTING POSITION, STATED RATHER THAN ASSUMED. Moving this
      // section to the end fixed what it did to others and exposed what others
      // do to it: section 2 opens every view in turn, so by the time this runs
      // the rail has already been scrolled to the far end and there is nothing
      // to its right. The first assertion below is about what the rail says
      // when there IS more to the right, so this is where that is arranged.
