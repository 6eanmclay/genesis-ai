...
**The rule going forward.** A registry that mirrors another must carry a runtime cross-check asserting every referenced name resolves in the registry it mirrors. All sixteen above are now guarded — `scripts/verify-storefront-scope.ts`, `scripts/verify-dimensions.ts`, `scripts/verify-taxonomy.ts`, `scripts/verify-workspace-context.ts`, `scripts/verify-action-sections.ts`, `scripts/verify-cognitive-proposals.ts`, `scripts/verify-initial-restraint.ts`, `scripts/verify-owner-facing-labels.ts`, `scripts/verify-field-labels.ts`, `scripts/verify-purchase-catalog.ts`, `scripts/verify-provenance.ts`, `scripts/verify-belief-review.ts`, `scripts/verify-tool-policy.ts`, `scripts/verify-tool-handlers.ts` — and all sixteen are currently clean.
