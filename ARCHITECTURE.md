...
| `EVENTS` (`lib/telemetry/taxonomy.ts`) → the emit calls in `lib/` and `app/` | **Found 2026-09-01.** The registry declares what Genesis may say about itself; the emit sites are what it actually says, and nothing compared them. THREE of fourteen declared events had never fired anywhere — `webhook.processed`, `creation.product_created`, `creation.design_saved`. A dead declaration is worse than an absent one in a specific way: anything built on the registry renders a category with a zero in it, and a zero reads as "this never happens" rather than "this was never measured". Cross-checked now by `scripts/verify-telemetry-gaps-db.ts`, which sweeps both trees WITH COMMENTS STRIPPED — the prose in those files names every event, so a sweep that counted comments would report each one wired the moment somebody wrote about it |
...
**The rule going forward.** A registry that mirrors another must carry a runtime cross-check asserting every referenced name resolves in the registry it mirrors. All eighteen above are now guarded — `scripts/verify-storefront-scope.ts`, `scripts/verify-dimensions.ts`, `scripts/verify-taxonomy.ts`, `scripts/verify-workspace-context.ts`, `scripts/verify-action-sections.ts`, `scripts/verify-cognitive-proposals.ts`, `scripts/verify-initial-restraint.ts`, `scripts/verify-owner-facing-labels.ts`, `scripts/verify-field-labels.ts`, `scripts/verify-purchase-catalog.ts`, `scripts/verify-provenance.ts`, `scripts/verify-belief-review.ts`, `scripts/verify-tool-policy.ts`, `scripts/verify-tool-handlers.ts`, `scripts/verify-social-creation.ts`, `scripts/verify-telemetry-gaps-db.ts` — and all eighteen are currently clean.
...
## Standing invariant: no autonomous mutation without explicit authorization (2026-09-12)

**Owner presence can provide authentication and context, but never silently substitutes for revoked autonomous authority.** Being signed in and in the conversation establishes *who* Genesis is talking to. It does not establish that they have authorised Genesis to change the business without asking. Presence ≠ permission.

There is one authorisation question — `autonomyAuthorizedFor(storeId, actionType)` in `lib/execution/genesisAutonomy.ts`: the action's own `maxAuthorityTier` (through which the category ceiling reaches) plus an active, un-revoked `DelegatedAuthority` row. **Both autonomous paths call that one function**, deliberately, so they cannot drift apart again. Presence decides only the *warrant* an execution is recorded under, never whether it may happen.

**This was not true until 2026-09-11, and the section below described the model rather than the code.** `tryExecuteAutonomousAction` was indeed never reachable from chat — but `app/dashboard/ai-actions.ts`'s own `proposeAction` had a second auto-execute gate reading `authorizationTier` alone, with no grant lookup anywhere in the file. An owner who pressed "Ask before publishing SEO changes" (which writes a revocation) still got SEO published the moment they mentioned it in conversation. The doc's claim about that *function* stayed true while its claim about the *system* had quietly stopped being. A future reader should take the general lesson: "X is never reachable from Y" is a statement about one call path, and the system is the union of all of them.

**The migration decision, recorded because the alternative was tempting (2026-09-12).** Tightening the gate left every existing store unauthorised, since nothing had ever required a grant. Sean's decision: they stay that way. No migration, no one-time grant, no backfill, no forced conversational notification. **A previous implementation behaviour is not equivalent to an owner's authorization** — what Genesis did under a permission model that turned out to be wrong is evidence about the model, not consent from the owner. Reading a store's own autonomous history as grounds to keep going is precisely the inference this invariant forbids, and it is the same standing rule as never inferring that money arrived because an order exists. The authority surface (`/authority`) is the single source of truth for what is authorised; owners grant capabilities there when they want them, and new stores follow the identical model. Held by `scripts/verify-authority-warrants.ts` §3c, which also sweeps every migration and every file under `app/`/`lib/` to assert that the only things that can create a grant are the helper itself and the owner's own button.
...
**Autonomous vs. human-approved decisions are permanently distinguishable, everywhere history is shown.** `ApprovalRequest.decisionMode` (`"human"` | `"chat_auto"` | `"autonomous"`) is a frozen snapshot of *which warrant authorised this*, alongside `delegatedAuthorityId` (which specific grant, for a permanent answer to "why was Genesis allowed to do this," even after the grant is later revoked). **`chat_auto` was added 2026-09-11**: J4 acting while the owner was present, which had been recorded as `"human"` by leaving the default unset — so the Marketing page rendered "You approved this" over changes no owner had ever seen. A fourth kind, system/CRON execution, writes no `ApprovalRequest` at all and is identified by `ExecutionLog.actorType: "SYSTEM"`. It surfaces in three places: the Marketing page's "Recently handled" list (with a Revert action), `ActivityFeed`'s message text (— handled automatically, under authority you granted / — published during your conversation, with no approval step; joined from `ApprovalRequest` by `executionId` at read time, never by mutating the append-only `ExecutionLog` row itself), and `runCognitiveReview`'s own prompt context (`getRecentDecisionOutcomes`, `lib/businessModel/reasoning.ts` — the current replacement for the original `recentGenesisHistory` proxy, see *J4 Cognitive Architecture — Reason* above) — there, `decisionMode` is explicitly informational only (the prompt tells Claude as much): it never changes what Genesis is currently allowed to do, and a string of successful autonomous outcomes can never grant more authority than the owner explicitly chose to give.
