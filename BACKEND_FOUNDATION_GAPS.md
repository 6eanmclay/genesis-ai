...
| # | Item |
|---|---|
| **23** | **PARTLY CLOSED 2026-09-02 — and the gap was bigger than this entry recorded.** It named the sixty-one suites that bring their own Postgres. Asking the lane functions for every `verify-*` file found those, and **another 106 that need no infrastructure at all** — pure functions, source assertions, vocabulary checks — equally unrun. 284 suites, of which 117 had a runner. **`scripts/run-code-suites.ts` is the third runner the right fix called for**, taking the cheap half first: `isCodeOnly` joins `needsDatabase` and `httpLane` in `scripts/lib/suiteLanes.ts` so a lane is still DERIVED, not listed, and `verification-inventory.ts` was updated in the same change — it counts what runs, and leaving it reporting 167 unrun on the day 106 started running would have been the exact drift its own comments warn about. **The 61 real-Postgres suites remain open**, and are what is left of this item. | **The first run found four failing suites, and every one was a real finding.** Three were the same underlying defect: `create_promotion` and `update_promotion` have been registered and executable since the promotions milestone with **no owner-facing wiring at all** — no `ACTION_SECTIONS` entry (so a pending promotion raised no badge on the nav item that owns it, and a deep link had nowhere to go), no workspace in `workspaceContext.ts` (so J4 did not know what the owner was looking at when it sent them there), and not one `FIELD_LABELS` entry (so an approval card offered the owner `discountType` and `amountOffInCents` as headings). The fourth was `verify-bag-checkout.ts` reading a file the Stripe webhook logic had moved out of — all six patterns found intact in `lib/payments/stripeEvent.ts` before the assertion was repointed, so nothing had regressed; the assertion had simply never had to be right. **Every repaired assertion was then sabotaged and each turns red**, including the two whose guarantee had got STRONGER since they were written (a partner-fulfilled order is no longer offered postage) — pinned rather than loosened. **One thing recorded rather than built**: `productIds` is hidden rather than labelled, because labelling it would put a list of cuids on an approval card under a friendly heading. Resolving those ids to product names is a real improvement and a design decision, not a label fix. |
