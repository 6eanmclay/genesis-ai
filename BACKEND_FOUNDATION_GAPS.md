...
| **27** | **A security assertion fails intermittently under lane load, and I have NOT root-caused it.** Found 2026-09-03 in the first full regression after gap 26 closed. `verify-order-webhook-live` failed on `it lands in the attacker's own store, where the money is` — expected 1, actual 0 — at 49s in the lane. It passes alone 4/4 (23s, 26s, 31s, 33s) and the lane passed 8/8 in the four runs before it, so it is load-dependent and rare. | **What the failing run actually showed.** The webhook was acknowledged (200) and the VICTIM's store correctly held 0 orders; only the attacker's own row was missing. The route has no `after()`, `waitUntil` or fire-and-forget write, so a 200 ought to mean the order was already persisted — which makes 'the test read too early' the LESS likely explanation and 'the handler took an acknowledge-without-creating branch' the more likely one. Neither is established, and I am not going to write down a mechanism I have not seen: that is exactly what gaps 25 and 26 each punished. | **Why it deserves its own item rather than a footnote.** This suite is one of the NINE in gap 25's blast radius — before 2026-09-03 it could not report a failure at all. So this may have been failing intermittently for weeks while the lane reported green, and 'it is probably just flaky' is precisely the reasoning that would have kept it hidden. It is an assertion about money landing in the wrong merchant's store. | **To investigate**: capture the handler's own logs on a failing run rather than the suite's assertion, since the question is which branch the route took, and that is not visible from the outside. |
