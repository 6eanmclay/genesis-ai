...
| ~~**E25**~~ | **RESOLVED 2026-09-09 by Sean, who decided the count rather than letting anyone edit the lock to match the code.** The rooms are **five** — Business, Storefront, Studio, Commerce, Account — and **Office is part of Business, not a sixth room**; the sixteen-item rail proposed by that day's Office references is explicitly not adopted. This entry sat red from 2026-09-02 and said exactly what was required: "either the rooms model is now five and GENESIS_SURFACES.md plus navConfig.ts's comment should say so (one line each, and the suite's expected list updates with them)". All three are now done, in that order and only after his decision. | `verify-rooms.ts` now asserts five and the five labels; its J4 assertion is untouched and remains the load-bearing half — five tabs is a room count, whereas an *Office tab* would still break the architecture, because J4 is who comes with the owner rather than a place they go. `navConfig.ts` had claimed "Your Business is gone from this list" for a week while Business sat first in that very list. |
| **E26** | **Money is written with a hardcoded dollar sign in the Creation Station.** `app/b/[slug]/studio/create/CreationStation.tsx:848` renders a supplier cost as `$${(variant.costInCents / 100).toFixed(2)}` instead of going through `formatMoney(cents, currency)`. Found by `verify-store-currency.ts`, whose whole purpose is that money becomes a string in exactly one place — the same rule whose earlier violation this repository already records as "a hardcoded dollar sign made that figure a claim about which money left their account". A second line flagged in `CreationStationClient.tsx:68` is a form-field unit conversion with no symbol at all, and is arguably a false positive of the sweep rather than a leak. | A merchant whose store is in GBP or EUR is shown their supplier cost in dollars. **Not fixed here, deliberately, and it is not a decision — it is a scope boundary**: the component has no currency to hand, so the fix threads the store's currency from the page through `CreationStationClient` into `CreationStation`, and Product Creation is recorded as the Studio reference implementation not to be modified outside that milestone. It is a small, contained change for whoever picks Studio up next. The suite is left FAILING so it stays visible. **AMENDED 2026-09-04.** An accepted-red suite hides everything that lands behind it, and three more leaks had. The sweep was flagging **nine** lines, not the two written up here: `lib/businessModel/digest.ts:137` (mine, 2026-09-03) rendered a promotion as `"5.00 off"` — no currency at all — into the digest fed to J4 as business context every turn, so a model handed an unlabelled figure would supply a symbol itself; and `lib/businessModel/businessMap.ts:352,394` put a bare product price and a bare "Spent with you" total on the cards an owner opens. All three are FIXED: `BusinessUnderstanding` now carries the store's `currency` (`string | null`, never defaulted to USD — inventing one is the failure this rule exists to stop), and both call sites go through `formatMoney`, omitting the fact entirely when there is no store row rather than guessing. `verify-business-map-db` asserted a bare `"42.00"`; updating that literal to `"$42.00"` alone would have left the same hole one symbol wider, so it now changes the store to GBP and asserts the owner reads `£42.00` — under a hardcoded `$` the old assertion still passes and only the new one fails. **The six that remain are now classified, so this is a work order rather than a count.** **Two are the real bug**, both inside the Studio boundary and both the same defect: `CreationStation.tsx:848`, and the helper behind it, `lib/creation/garment.ts:73`, whose `formatCents` hardcodes `$`. **That helper is the more dangerous half.** `formatCents` is not one function — `app/dashboard/understanding/page.tsx:20` and `app/j4/J4Surface.tsx:44` each define `const formatCents = formatMoney`, currency-aware and taking a currency argument, while `garment.ts` *exports* a one-argument `formatCents` that always says dollars. The same name means two different contracts depending on the module, and Studio imports the wrong one. Renaming it is most of the fix. **Four are false positives of the sweep, not leaks**, and each already uses the right currency: `lib/bag/providerLines.ts:109` builds the decimal string PayPal's API requires — the same reason `app/store/[slug]/actions.ts` is already exempt; `app/dashboard/orders/PackingSlip.tsx:61` and `lib/payments/financials/presentation.ts:19` both pass a real currency into `Intl.NumberFormat` (the order's store currency, and the provider's own per-currency balance) and hardcode only the `en-US` *locale*; and `CreationStationClient.tsx:68` is the form-field unit conversion already noted above. **I did not add exemptions for those four.** An exemption stops the sweep watching a file, and the two `Intl` call sites genuinely do turn cents into a display string — exempting them would mean a future hardcoded currency there goes unseen. The principled fix is for them to go through `lib/money`, which today has no thousands separator (`formatMoney` is symbol + `toFixed(2)`), so adopting it would change `$1,234.56` to `$1234.56` on a packing slip. Giving `formatMoney` grouping would fix that and is invisible below 1000 — but it changes how money reads product-wide, which is Sean's call, not a side effect of clearing a sweep. |

| **E27** | **The persistent J4 is not the calm artwork the rule requires.** Sean, 2026-09-05: J4 is like a dog when its person comes home — the entrance is the excited greeting, then he settles and is calm and present, and the black/honeycomb version is his normal working state with no green energy around him. On 2026-09-09 he supplied the new angular-emblem J4 and, asked directly, chose to keep that split: *"Keep calm/greeting split; I'll derive a calm variant."* **I could not derive one.** Two independent reasons, both measured rather than judged. First, the character reaches the frame edge — the bottom-left corner of `j4-v2.png` is white armour (234,234,233) — so any mask that darkens the background cuts into him, against the standing instruction to preserve the master pixels. Second, the render is not calm and is not close: `verify-j4-calm.ts` measures the ground beside his shoulder at **97.2**, where the threshold is 40, the retired calm badge read **26**, and the **greeting** artwork — the picture the rule exists to keep out of the corner — reads **74.9**. The persistent J4 is now brighter than the thing the rule excluded. | **The suite is left FAILING and is accepted-red, rather than the threshold being moved.** `verify-rooms.ts` is this repository's precedent: it was left failing for a week rather than edited, "because a lock quietly rewritten to match the code is not a lock." The measurement is split into its own suite so accepting it costs exactly one guarantee — `verify-j4-artwork` keeps the clean visor and the entrance invariants and stays green. E26's own entry records what happens otherwise: three further currency leaks landed behind an accepted-red suite and went unseen. **To close it Sean supplies a calm master** — the same J4 on a black ground with a faint honeycomb and no glow — it replaces the persistent asset, and the suite passes without a line of it changing. Nothing else is blocked: the new J4 ships everywhere `J4Character` renders, with the clean visor and the emblem placement he asked for. |

| **E28** | **The shipped J4 render predates the canonical visual system.** Sean locked three separate assets on 2026-09-10, and the point of the lock is that they are not interchangeable: the CHARACTER (white/black body, clean black visor, J4 ear modules); the HELMET INSIGNIA, a black **diamond platform** carrying the continuous emblem — the version with no central separation, which is what gives it the hourglass geometry, and the same emblem used as the physical suit lapel pin; and the EAR-MODULE MARK, which is Sean's own Beats-by-Dre analogy and the right one — a signature hardware identifier on the ear cup, whose exact angles, line weight, orientation and negative space **are** the asset. Compared against the signed-off brand sheet, `public/brand/j4-v2.png` matches neither mark: the helmet carries an open angular mark on a black panel with **no diamond platform at all**, and the ear carries a mark from the same family as its own forehead mark rather than the sharp J-and-triangle from the close-up. So the shipped character has two variants of ONE approximate mark where canon calls for two distinct ones. Found by cropping and looking at the actual render rather than by trusting that a file named `j4-v2.png` existed — which is the standing invariant in ARCHITECTURE.md applied to artwork: a file on disk proves persistence, not appearance. | **`verify-j4-assets.ts` is left FAILING and accepted-red rather than the check being written to match what shipped.** Same precedent as E27 and, before it, `verify-rooms`: a lock quietly rewritten to match the code is not a lock. The suite guards a single fact so accepting it costs exactly one guarantee — the other fourteen checks stay green and keep working, including the byte-for-byte fingerprints of the three approved masters, the rule that the helmet and ear marks can never be swapped, and the rule that the visor gets nothing. **This is not something code can close.** It needs J4 re-rendered against the locked system: diamond insignia on the helmet centre, the signature mark on the ear module, clean visor. When that render arrives it replaces `j4-v2.png`, its fingerprint goes into `RUNTIME.character.sha256` in `lib/brand/j4Assets.ts`, and the suite passes without a line of it changing. Sean, on why the fingerprint rather than a similarity check: "I don't want 'close enough', 'similar J4', 'another interpretation of the logo', or an image generator deciding that the logo should look different. Exact geometry matters." A similarity check would pass the exact failure mode being guarded against. |

## Needs a migration and a decision about existing data

| # | Item | Exact behaviour today | What a safe migration requires |
|---|---|---|---|
| ~~**E11**~~ | **RESOLVED 2026-09-02 — and it turned out not to need a decision at all.** This entry set out three steps and said the middle one, deciding what happens to colliding accounts, was a product question with no safe default. **Step 1 was run first, read-only, exactly as this entry prescribed, and it emptied step 2**: production holds 40 accounts, **0 with any uppercase character, 0 case-insensitive collisions, 0 with surrounding whitespace**. There are no collisions to decide about, and — the part that mattered — every stored address already equals its own normalised form, so no existing row changes meaning. That is the precondition the reverted August attempt lacked, not an argument against it. **Step 3 shipped in one deploy**: `normalizeEmail` (trim + lowercase, deliberately not clever — a plus-tag and gmail dots survive) applied to registration's write, the sign-in lookup, the throttle lookup, the password-reset lookup, and the member lookup that had been keeping its own private copy of the rule. **No migration was needed**: two registrations differing only by case now write the same string, so the existing `@unique` constraint refuses the second — the case-insensitive index this entry called for would only have defended against a future caller forgetting, which is asserted in the suite instead. Proven over real HTTP against a real server and a real Postgres, because both halves live behind a request scope: an account signs in with capitals it was not created with, registration stores one form whatever is typed, and the same address in different capitals is refused a second account. | **One limit, stated rather than implied**: a row stored WITH capitals is no longer reachable by the normalised lookup. Production had none — that measurement is the whole reason this shipped — and the suite asserts that behaviour directly so it is a known property rather than a surprise. **Still uncovered, deliberately**: the Google provider writes through `PrismaAdapter`, which stores whatever Google returns and links by that same value. Self-consistent, and Google lowercases in practice, so the hole is theoretical — but wrapping an auth adapter changes the sign-in path in a way that could not be proven before shipping, and guessing at it unproven is worse than recording it here. |
