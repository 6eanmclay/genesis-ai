...
| **E25** | **The navigation has five primary tabs; the locked design says four.** Found 2026-09-02 by `verify-rooms.ts`, on the day it first had a runner. `GENESIS_SURFACES.md` is unambiguous — its title is "Four rooms", it says **"The rooms model is locked, 2026-08-15"**, and its own migration table reads "Your Business (Overview) | **The opening screen** | **Kept, but not a room.**" `navConfig.ts`'s comment still says the same thing in the present tense: "Arrival is not a room either... That is why 'Your Business' is gone from this list." **It is not gone from that list.** `PRIMARY_TAB_COUNT` is 5 and the tabs are Business, Storefront, Studio, Commerce, Account. The Business tab arrived in `b75a0b8` ("The map had no door") — a Business Map commit you approved and deployed, and the map genuinely did need an entry point. So this is very likely a deliberate amendment that was never written down, rather than a regression. **I am not deciding which.** | Nothing is broken for an owner: the tab works and the map is reachable. What is broken is that the frozen document and the shipped product disagree, and the suite that guards the lock is red — so the next person to read either one is misled. **What is required from you**: either the rooms model is now five and `GENESIS_SURFACES.md` plus `navConfig.ts`'s comment should say so (one line each, and the suite's expected list updates with them), or Business should not be a primary tab and the nav changes instead. I have left `verify-rooms.ts` FAILING rather than updating it, because it is correctly reporting a real divergence and quietly editing a lock to match the code is how a lock stops meaning anything. |
