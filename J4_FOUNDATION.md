...
**Status: v2, 2026-08-05 — the authoritative specification, brought back into alignment with reality.** v1 (2026-08-04) proposed closing two gaps; this pass audited that proposal against the real code and everything frozen since, and found three things worth correcting: **Gaps A and B are fully closed** (confirmed live in code, not just planned — see below), **a real new gap (C) exists** that didn't when v1 was written (Growth Points/plan/subscription/trial-state, real only as of 2026-08-05's pricing freeze, currently absent from Business Understanding), and **one claim made elsewhere in this codebase overstated what's actually retrievable today** (corrected as Gap D). Accuracy over preserving earlier assumptions — this document describes what exists, not what was once proposed.
...
**Gap C — OPEN, found 2026-08-05.** The store's own relationship with the platform — Growth Points balance, current `Plan`, subscription status, Business Partner trial state — is real (the Growth Points economy's pricing froze 2026-08-05, a day after v1 of this document) but **not part of `BusinessUnderstanding`**. Checked directly: both Reason (`cognitiveLayer.ts`) and chat (`ai-actions.ts`) *do* each fetch `store.growthPointBalance` today — but independently, hardcoded at their own call sites, not through the canonical understanding object. That's precisely the duplicated-assembly pattern Gap A was built to eliminate, recurring for a data category that didn't exist when Gap A closed. Worse: **neither surface has access to which plan a store is on, its subscription status, or trial state, at all** — no call site anywhere fetches these today. This is directly load-bearing: `J4_IDENTITY.md`'s "J4 is a trusted advisor" principle requires J4 to understand how an owner is actually using Genesis to recommend a fitting plan — today there's no data path for that beyond a raw point balance, duplicated in two places.

**Gap D — OPEN, found 2026-08-05, corrects an overstated claim.** `J4_IDENTITY.md`'s "relationship continuity" principle uses the example *"we ruled this out six months ago because…"* and states this is *"a real, existing fact this system can already answer, not a new capability to build."* That overstates it. `getRecentDecisionOutcomes` — the function that would answer this — defaults to a **14-day window** (§1 above). `getEntityHistory` can pull a specific record's full unbounded timeline, but only if the caller already knows which record; recalling a past *decision by topic*, months back, isn't something `BusinessUnderstanding` supports today. Long-term *pattern* memory (`Belief`) is real and genuinely unbounded — a belief that solidified from evidence six months ago stays real today. Long-term *specific decision* recall is not. `J4_IDENTITY.md` should be corrected to reflect this distinction, not just this document.

## What a J4 Foundation milestone should build next

Gaps A and B are done — no further work. For C and D, deliberately small, matching the same discipline v1 held itself to:

1. **Extend `BusinessUnderstanding` with a real `platformRelationship` field** — `planId`/`planName`, `growthPointBalance`, `subscriptionStatus`, `businessPartnerTrialEndsAt`. Every one of these fields already exists on `Store` today; this is routing, not new schema. Once real, `cognitiveLayer.ts`'s and `ai-actions.ts`'s own ad hoc `growthPointBalance` fetches should be replaced with this field, closing the duplication the same way Gap A closed it for facts/beliefs.
2. **Gap D is a real, open product question, not an implementation default** — how far back should decision-memory reach, and should it be a wider fixed window, or a real topic-searchable lookup instead of a time window at all? Not decided here; a real number or mechanism is Sean's own call, the same discipline every other real number in this project has followed. Until decided, `J4_IDENTITY.md`'s "six months ago" example should read as a real future capability being designed toward, not a description of what exists.
