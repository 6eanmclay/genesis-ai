...
**Status, 2026-08-23 — the ask now reaches the owner.** `proposeConnectionGaps` was shipped where it could not be heard: it wrote a `CognitiveOutput`, which surfaces on the Connections page, so the ask only ever reached an owner who had already gone looking for it. This document said the rest was "the natural next step once a real moment to trigger one exists" — Proactive J4 (`lib/intelligence/proactive.ts`) is that moment. The same gap is now also a `GenesisObservation`, which is what J4 speaks, so the frozen example sentence reaches the conversation once, at the moment the gap matters, and stops when the gap closes. A second real instance also exists and predates this: `raiseEconomicsQuestions` (`lib/sourcing/economicsQuestions.ts`) asks for a specific missing FACT — what a supplier charges — gated on real evidence, and threads it into the turn context.

**Still genuinely unbuilt:** asking for a specific missing *document* (a lease, an employee handbook). That needs a product decision about which documents matter and what evidence justifies each, which is not something this document settles.

**The earlier status line, kept for its reasoning**: `proposeConnectionGaps` (`lib/integrations/gaps.ts`) is a real, already-shipped instance of this principle — evidence-based integration recommendations, grounded in a specific observed gap, not a generic "connect everything" nudge. Asking for a specific missing *document* or *fact* (a lease, a handbook, an inventory count) the way the other three examples describe is not yet built — real future work, the natural next step once a real moment to trigger one exists, named here so it isn't lost, matching this document's own "Deliberately unbuilt" discipline below.
...
This is the product-experience expression of what J4's `Learn`/`Understand` architecture does technically, and as of 2026-08-18 it is true without qualification. Long-term *pattern* memory (`Belief`) was always unbounded. Long-term *specific decision* recall now is too: `findRelevantDecisions` (`lib/businessModel/reasoning.ts`) searches every decision the owner has made, at any age, ranked by relevance to what they actually asked, with recency as a small nudge rather than a cutoff. Both conversational paths supply it. *"We ruled this out six months ago because…"* is a real answer now, from the decision record itself rather than only from a pattern that happened to rise to a `Belief`. This paragraph previously carried a correction saying the opposite, which was accurate for as long as `getRecentDecisionOutcomes`' 14-day window was the only store-wide decision read; that function still exists and is still windowed, because "what has been settled lately" and "did we decide about X" are different questions. Verified end to end against a real store with a decision aged 210 days: J4 answered "back in January we looked at renaming the shop to Fernbrook Apothecary, and you decided against it", including the owner's own stated reasoning.
...
- ~~No mechanism proactively asks for a specific missing document or fact at the moment it would matter.~~ **Partly closed 2026-08-23.** Asking for a missing *fact* is built twice over — `proposeConnectionGaps` (a missing integration) and `raiseEconomicsQuestions` (what a supplier charges), both evidence-gated — and both now reach the owner in conversation rather than only on a page they had to visit. Asking for a specific missing *document* remains unbuilt and needs a product decision about which documents matter.
