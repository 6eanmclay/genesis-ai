...
} from "@/lib/dashboard/storeChatPrompts";
import {
    ...
    async function proposeAction(actionType: string, proposedInput: unknown, summary: string) {
      ...
      const approval = await prisma.approvalRequest.create({
        data: {
          ...
          groupId,
          // AI Cost & Usage Infrastructure — correlated to PRIMARY's own
          // event, not secondary/composition's: PRIMARY is what actually
          // decided which fields are even in scope this turn
          // (touchesTheme/touchesSecondaryContent), the same "one turn,
          // one bundle" grouping groupId above already treats these
          // proposals as, presentationally. A field literally generated by
          // secondary/composition attributed to primary's call is a
          // deliberate, bounded approximation (see the AI Cost & Usage
          // Infrastructure plan's Milestone 4 scope), not an attempt at
  ...
  try {
    ...
    await prisma.approvalRequest.create({
      data: {
        ...
        decidedByUserId: userId,
        decidedAt: new Date(),
        // No new AI call happens on a revert — this row undoes the
        // original using its own already-recorded values, so the honest
        // correlation is the same AI work the original approval came
        // from, not a fresh one.
        aiUsageEventId: original.aiUsageEventId,
      },
    });
